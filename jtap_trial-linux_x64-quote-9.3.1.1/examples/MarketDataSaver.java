...
public class MarketDataSaver {
        ...
        public void run() {
            ...
            while(!requestStop){
                ...
                if ( field==null )
                    continue;
                try {
                    String contractUID = field.Contract.Commodity.ExchangeNo + "." + field.Contract.Commodity.CommodityNo + field.Contract.ContractNo1;
                    BufferedWriter writer = dataWriterMap.get(contractUID);
                    if ( writer==null ){
                        writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(dataDir,contractUID+"."+dataFileSuffix),true),"UTF-8"));
                        writer.write("\n");
                        dataWriterMap.put(contractUID, writer);
                    }
                    line.setLength(0);
                    
                    line.append(field.DateTimeStamp)
                        .append(",").append(field.Contract.Commodity.ExchangeNo)
                        .append(",").append(field.Contract.Commodity.CommodityNo + field.Contract.ContractNo1)
                        .append(",").append(field.QTotalQty)
                        .append(",").append(field.QLastQty)
                        .append(",").append(price2str(field.QLastPrice))
                        .append("\n");
        ...
        throws Throwable
    {
        ...
        mdApi.setListener(new QuoteApiListener() {
			...
			@Override
			public void OnRspQryCommodity(int sessionID, int errorCode,
					byte isLast, TapAPIQuoteCommodityInfo info) {
			    //System.out.println("OnRspQryCommodity session "+sessionID+" error "+errorCode+" "+info);
			}



			@Override
			public void OnRspQryContract(int sessionID, int errorCode,
					byte isLast, TapAPIQuoteContractInfo info) {
                //System.out.println("OnRspQryContract session "+sessionID+" error "+errorCode+" "+info);
			...
			@Override
			public void OnRspSubscribeQuote(int sessionID, int errorCode,
					byte isLast, TapAPIQuoteWhole info) {
			...
			@Override
			public void OnRspUnSubscribeQuote(int sessionID, int errorCode,
					byte isLast, TapAPIContract info) {
        ...
        for(int i=0;i<ids.length;i++)
        {
            ...
            System.out.println("Subscribe exchange "+exchangeId+" commodity "+commodityNo+" contract "+contractNo);
